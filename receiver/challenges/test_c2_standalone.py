...
def run_c2(commands, timeout=10):
    if isinstance(commands, str):
        commands = [commands]
    cmd = [
        "docker",
        "exec",
        "-i",  # keep STDIN open for the process
        "c2_container",
        "timeout",
        str(timeout),
...
def check():
    try:
        ...
        binary_out = run_c2(["exit"], timeout=5)
        print(binary_out)
        ...
        process_out = run_c2(["process test_data", "exit"], timeout=10)
        print(process_out)
        ...
        diag_out = run_c2(["diagnostic", "test_diag", "exit"], timeout=10)
        ...
        classified_out = run_c2(["classified", "test_classified_data", "exit"], timeout=10)
        ...
        officer_out = run_c2(["officer", "Test Officer", "Kapten", "exit"], timeout=10)
        ...
        network_out = run_c2(["network", "test_packet", "exit"], timeout=10)
        ...
        help_out = run_c2(["help", "exit"], timeout=10)
        ...
        safe_overflow_out = run_c2(["process " + "A" * 50, "exit"], timeout=15)
